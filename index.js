...
/*
CREATE TABLE `top_franquias`.`youtube_scrapper` (
  ...
  ALTER TABLE `top_franquias`.`youtube_scrapper` 
...
async function updateNewVideos() {
    ...
    (async () => {
        ...
        connection.query(getVideosDatabase, function (error, results, fields) {
            ...
            for (const key in results) {
                if (Object.hasOwnProperty.call(results, key)) {
                    const link = results[key];
                    scrapeYt.getVideo(link.url, {
                        type: "video"
                    }).then(videos => {
        ...
        feed.items.forEach(item => {
            let link = item.link.split('watch?v=')[1];
